...
spread_minimum_basis_points = 10
sleep_interval_minutes = 1
max_trade_value_gbp = 5000
...
while(value):
    ...
    # Parse GBP balance
    gbp_balance = float(resp.json().get('result').get('ZGBP'))
    print(this_file + ":\t" + "GBP balance is: " + str(gbp_balance))

    if gbp_balance > trade_minimum:
        print(this_file + ":\t" + "GBP balance is above trade minimum of GBP " + str(trade_minimum) + ". Computing trade.")
        # Compute bid price
        resp = requests.get('https://api.kraken.com/0/public/Ticker?pair=XBTGBP')
        market_data = (resp.json().get('result')).get('XXBTZGBP')
        ...
        # compute bid amount in GBP
        bid_gbp = int(min(gbp_balance, max_trade_value_gbp))
        print(this_file + ":\t" + "Proposed bid amount: GBP " + str(bid_gbp))

        # Compute bid volume
        bid_volume = bid_gbp / mybid 
